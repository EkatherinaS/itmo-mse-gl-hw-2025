...
void Model::initialize(QOpenGLContext * context, Camera * camera) {
    context_ = context;
    camera_ = camera;
}

void Model::addQOpenGLShaderProgram(QOpenGLShaderProgram * program) {
...
void Model::render(QOpenGLShaderProgram * program) {
    ...
    QMatrix4x4 model = camera_->getModel();
    QMatrix4x4 view = camera_->getView();
    QMatrix4x4 projection = camera_->getProjection();
    QMatrix4x4 mv = view * model;
    QMatrix4x4 mvpCamera = projection * view * model;

    program->setUniformValue("projection", projection);
    program->setUniformValue("mv", mv);
    program->setUniformValue("mvp", mvpCamera);
    program->setUniformValue("model", model);
    program->setUniformValue("viewPos", camera_->getPosition());
    program->setUniformValue("maxSize", maxSize_);
